...
function normalizeOptions(userOptions = {}) {
  ...
  const luauStyleRaw = typeof userOptions.luauStyle === "string"
    ? userOptions.luauStyle.toLowerCase()
    : "";
  const luauStyle = lang === "luau" && (luauStyleRaw === "v3" || luauStyleRaw === "moonless")
    ? "v3"
    : "default";
  ...
  const autoLuauHigh = lang === "luau" && (presetName === "high" || presetName === "v3");
  ...
  if (lang === "luau" && options.luauStyle === "v3") {
    options.compact = true;
    options.cff = true;
    options.cffOptions.mode = "vm";
    options.dead = true;
    options.wrap = false;
    ...
    options.padFooterOptions.blocks = Math.max(options.padFooterOptions.blocks, 2);
    options.renameOptions.homoglyphs = true;
    options.vm.enabled = true;
    options.vm.topLevel = false;
    options.vm.layers = Math.max(options.vm.layers, 1);
    options.vm.fakeOpcodes = Math.max(options.vm.fakeOpcodes, 0.35);
    options.vm.blockDispatch = true;
    options.vm.dispatchGraph = "sparse";
    options.vm.stackProtocol = "api";
    options.vm.isaPolymorph = true;
    options.vm.fakeEdges = true;
    options.vm.numericStyle = "chaos";
    options.vm.decoyRuntime = true;
    options.vm.symbolNoise = true;
    options.vm.instructionFusion = true;
    options.vm.semanticMisdirection = true;
    options.vm.dynamicCoupling = true;
  }
